...
// linux/amd64) and runs scripts/build-cloudcli-office-preview-artifact.mjs inside it.
// The repo is bind-mounted to /repo, so .dockerignore does NOT limit what the build
// can read (it can see vendor/HolyClaude/... for the chained account-management
// patches). Mirrors build-cloudcli-account-management-artifact-container.mjs.
...
const buildCommand = [
  ...
  'node scripts/build-cloudcli-office-preview-artifact.mjs',
].join(' && ');
