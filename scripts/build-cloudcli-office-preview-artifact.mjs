...
// Mirror of vendor/HolyClaude/scripts/build-cloudcli-account-management-artifact.mjs,
// adapted for the AgentMate office-preview overlay. Differences:
//   - artifactDir is AgentMate-owned (patches/source/artifacts)
//   - the patch set chains HolyClaude's account-management overlay FIRST (so the
//     rebuilt frontend keeps local account management), then this overlay
//   - manifest records both origins + a dist-overlay removal clause
...
const artifactFile = `cloudcli-ai-cloudcli-${packageVersion}-agentmate-office-preview.tgz`;
...
// Combined, ordered patch set: HolyClaude account-management overlay first (so the
// rebuilt bundle keeps HolyClaude's local account management), then this overlay.
// Each entry is { origin, dir, file, abs }.
function collectPatches() {
  const patches = [];
  for (const [origin, dir] of [['holyclaude', holyclaudePatchDir], ['agentmate', officePatchDir]]) {
...
const workdir = await mkdtemp(path.join(tmpdir(), 'agentmate-cloudcli-office-'));
try {
  ...
  const manifest = {
    bridge: 'cloudcli-office-preview',
    ...
    installStrategy: {
      ...
      reason: 'Only dist/ is overlaid onto the installed CloudCLI; dist-server/ is untouched so all seven HolyClaude runtime patches (which write only to dist-server/ or an external plugin dir) keep working with zero re-application.',
    },
    verification: {
      detector: 'scripts/verify-cloudcli-office-preview-support.mjs',
      expectedState: 'agentmate-bridge-complete',
      minificationSafeMarker: 'data-agentmate-office-preview',
    },
    upstreamRefs: [
      'https://github.com/xushanpei/open-file-viewer',
      'https://www.npmjs.com/package/@open-file-viewer/core',
    ],
    removal: 'Remove when CloudCLI ships native docx/xlsx/pptx inline preview (detector reports upstream-complete), or when @open-file-viewer/* is swapped for another renderer. Then delete patches/source/cloudcli-office-preview/, scripts/build-cloudcli-office-preview-*, patches/source/artifacts/cloudcli-*office-preview* + cloudcli-office-preview.manifest.json, and the Dockerfile Layer A stage.',
  };

  writeFileSync(
    path.join(artifactDir, 'cloudcli-office-preview.manifest.json'),
    `${JSON.stringify(manifest, null, 2)}\n`,
  );

  console.log(`[cloudcli-office] wrote ${artifactPath}`);
} finally {
  if (!keepWorkdir) {
    await rm(workdir, { recursive: true, force: true });
  } else {
    console.log(`[cloudcli-office] kept ${workdir}`);
  }
}
